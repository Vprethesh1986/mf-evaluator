...
function scoreAnswers(answers) {
  ...
  if(isHelp){recommendation="SEE RECOMMENDATIONS BELOW";recColor="#63b3ed";recReason="Based on your profile, we have curated the top 5 funds best suited for you.";}
  ...
  else if(isIndex&&finalScore>=5){recommendation="BUY / ACCUMULATE";recColor="#22c55e";recReason="Index funds offer low-cost, diversified market exposure. Given your profile, this is a solid core holding.";}
  else if(isHybrid&&finalScore>=4&&finalScore<=7){recommendation="BUY / ACCUMULATE";recColor="#22c55e";recReason="Hybrid funds balance equity and debt dynamically \u2014 well-matched to your balanced profile.";}
  else if(finalScore>=7&&!isDebt){recommendation="BUY / ACCUMULATE";recColor="#22c55e";recReason="Your aggressive growth profile and long horizon make this equity fund a suitable choice for wealth creation.";}
  ...
  else if(isFirstTime&&finalScore>=5){recommendation="BUY";recColor="#22c55e";recReason="A solid first investment aligned with your profile. Start with SIP mode to average out entry cost.";}
  else{recommendation="HOLD / REVIEW";recColor="#eab308";recReason="Your profile and fund type are broadly aligned. Review expense ratio and 3-year rolling returns against category average before committing.";}

  let taxNote="";
  if(answers.q8&&answers.q8.includes("30%")){
    const isElss=!isHelp&&/elss|tax saver|tax saving/i.test(fund);
    const isEquityFund=!isDebt;
    taxNote=isElss?"ELSS funds qualify for Section 80C deduction up to \u20b91.5L \u2014 tax-efficient for your 30% bracket.":isEquityFund?"For your 30% bracket, LTCG > \u20b91.25L is taxed at 12.5%. Consider tax harvesting annually.":"Debt fund gains are taxed as per your slab (30%). Consider tax-free alternatives or ELSS.";
  }else if(answers.q8&&answers.q8.includes("20%")){taxNote="At 20% slab, equity LTCG above \u20b91.25L is taxed at 12.5%. Debt fund gains are slab-taxed.";}
...
function FundSearch({value,onChange}){
  ...
  const clear=()=>{onChange("");setQuery("");inputRef.current&&inputRef.current.focus();};
  const handleKey=(e)=>{
    if(!open)return;
  ...
  const highlight=(text,q)=>{
    ...
    return React.createElement(React.Fragment,null,text.slice(0,idx),React.createElement("mark",{style:{background:"rgba(212,160,23,0.3)",color:"#d4a017",borderRadius:"2px"}},text.slice(idx,idx+word.length)),text.slice(idx+word.length));
  };
  return(
    <div style={{position:"relative"}}>
      <div style={{fontSize:"11px",color:"#d4a017",fontWeight:"500",marginBottom:"10px",letterSpacing:"0.5px"}}>{ALL_FUNDS.length.toLocaleString()} funds available</div>
      {value==="__HELP__"?(
            ...
            <div ref={dropRef} style={{position:"absolute",top:"calc(100% + 6px)",left:0,right:0,background:"#1a1a2e",border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:"10px",maxHeight:"240px",overflowY:"auto",zIndex:999,boxShadow:"0 16px 48px rgba(0,0,0,0.5)"}}>
              {matches.map((f,i)=>(
                <div key={i} onMouseDown={()=>pick(f)}
                  ...
                  onMouseEnter={()=>setFocused(i)}>{highlight(f,query)}</div>
              ))}
              {ALL_FUNDS.filter(f=>query.toLowerCase().split(" ").every(w=>f.toLowerCase().includes(w))).length>80&&(
                <div style={{padding:"10px 14px",fontSize:"12px",color:"#8a8278",textAlign:"center"}}>Showing 80 of many results \u2014 type more to narrow</div>
              )}
            </div>
          )}
...
function ResultDashboard({answers,result,suggestions,onReset}){
  ...
  return(
    <div style={{maxWidth:"760px",margin:"0 auto",padding:"0 20px 80px",animation:"fadeIn .5s ease"}}>
      ...
      {/* CHANGE 2: Help Me Choose top funds */}
      {isHelp&&suggestions&&suggestions.length>0&&(
        <div style={{background:"rgba(99,179,237,0.05)",border:"1px solid rgba(99,179,237,0.2)",borderRadius:"16px",padding:"28px",marginBottom:"20px"}}>
          ...
          {suggestions.map((fund,i)=>(
            <div key={i} style={{display:"flex",alignItems:"flex-start",gap:"14px",padding:"14px 0",borderBottom:i<suggestions.length-1?"1px solid rgba(255,255,255,0.05)":"none"}}>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"16px",padding:"28px",marginBottom:"20px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"12px"}}>
          {[{label:"Amount Invested",value:fmt(projection.invested),color:"#a09890"},{label:"Estimated Returns",value:fmt(projection.gains),color:"#22c55e"},{label:"Final Corpus",value:fmt(projection.fv),color:"#d4a017"}].map(({label,value,color:c})=>(
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"16px",padding:"28px",marginBottom:"20px"}}>
        ...
        {[
          ...
          ["Risk",answers.q7],
          ["Tax Bracket",answers.q8&&answers.q8.split("\u2014")[0].trim()],
          ["Context",answers.q9],
        ].map(([k,v])=>(
      ...
      <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:"16px",padding:"24px",marginBottom:"20px"}}>
        ...
        <div style={{fontSize:"12px",color:"#6a6258",lineHeight:"1.9"}}>
          ...
          NAV of schemes may go up or down depending upon the factors and forces affecting the securities market including fluctuations in interest rates. The sponsor, AMC, trustees, their employees, and associates are not liable or responsible for any loss or shortfall resulting from the operation of the scheme.{" "}
      ...
      <button onClick={onReset} style={{width:"100%",background:"rgba(212,160,23,0.1)",color:"#d4a017",border:"1.5px solid rgba(212,160,23,0.3)",borderRadius:"10px",padding:"14px",fontFamily:"inherit",fontSize:"15px",fontWeight:"600",cursor:"pointer",transition:"all .2s",letterSpacing:"0.3px"}}
