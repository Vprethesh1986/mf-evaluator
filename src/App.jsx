...
// ── Curated Direct Plan funds for HELP ME CHOOSE ──
...
function suggestFunds(profile, answers) {
  ...
  const scored = CURATED_FUNDS.map(fund => {
    ...
    if (fund.profile.includes(profile)) score += 10;
      ...
      const closest = Math.min(...fundProfiles.map(fi => Math.abs(fi - investorIdx)));
      score += closest === 1 ? 5 : closest === 2 ? 2 : 0;
    }
    // ELSS bonus for 30% tax bracket
    if (isElssNeeded && fund.tags.includes("elss")) score += 4;
    // Debt preference for conservative
    ...
    // All curated funds are Direct plans — no penalty
    return { ...fund, matchScore: score };
  });

  // Sort by matchScore desc, then by category alphabetically for tie-break
  scored.sort((a,b) => b.matchScore - a.matchScore || a.category.localeCompare(b.category));

  // Keep only Direct plans — exclude any Regular plan
  const directOnly = scored.filter(f => /direct plan/i.test(f.name) && !/regular plan/i.test(f.name));

  // Deduplicate by category — max 2 per category for diversity
  const catCount = {};
  const top5 = [];
    ...
    <div style={{position:"relative"}}>
      ...
      {value === "__HELP__" ? (
        <div style={{display:"flex",alignItems:"center",gap:"10px",background:"rgba(99,179,237,0.08)",border:"1.5px solid rgba(99,179,237,0.4)",borderRadius:"10px",padding:"12px 14px"}}>
          <div style={{flex:1,fontSize:"14px",fontWeight:"600",color:"#63b3ed",lineHeight:"1.45"}}>🤝 Help Me Choose — suggestions will be shown after all questions</div>
        ...
        <>
          ...
          <div style={{marginTop:"16px",borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:"14px",display:"flex",alignItems:"center",gap:"10px"}}>
            ...
            <button
              ...
              style={{
                background:"rgba(99,179,237,0.1)",border:"1.5px solid rgba(99,179,237,0.35)",
                borderRadius:"8px",padding:"8px 14px",fontSize:"13px",fontWeight:"600",
                color:"#63b3ed",cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit",
...
function ResultDashboard({ answers, result, suggestions, onReset }) {
  ...
  return (
    <div style={{maxWidth:"760px",margin:"0 auto",padding:"0 20px 80px",animation:"fadeIn .5s ease"}}>
      ...
      {/* ── HELP ME CHOOSE: Fund Suggestions Panel ── */}
      {suggestions && suggestions.length > 0 && (
        <div style={{
          background:"rgba(99,179,237,0.05)", border:"1.5px solid rgba(99,179,237,0.25)",
          borderRadius:"14px", padding:"24px", marginBottom:"20px"
        }}>
          <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"18px"}}>
            <div style={{fontSize:"18px"}}>🤝</div>
            <div>
              <div style={{fontSize:"15px",fontWeight:"700",color:"#63b3ed"}}>Top 5 Suggested Funds for You</div>
              <div style={{fontSize:"12px",color:"#6a6258",marginTop:"2px"}}>Based on your investor profile: <span style={{color:"#d4a017",fontWeight:"600"}}>{result.profile}</span> — Direct Growth Plans only</div>
            </div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
            {suggestions.map((fund, i) => {
              const pct = Math.round((fund.matchScore / 14) * 100);
              const barColor = pct >= 80 ? "#22c55e" : pct >= 60 ? "#84cc16" : pct >= 40 ? "#eab308" : "#f97316";
              const categoryColors = {
                "Large Cap":"#63b3ed","Flexi Cap":"#a78bfa","Mid Cap":"#f59e0b",
                "Small Cap":"#f87171","Balanced Advantage":"#34d399","Aggressive Hybrid":"#fb923c",
                "Index - Large Cap":"#60a5fa","Index - Mid/Large":"#818cf8",
                "ELSS":"#4ade80","Short Duration Debt":"#94a3b8"
              };
              const catColor = categoryColors[fund.category] || "#8a8278";
              return (
                <div key={i} style={{
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:"12px"}}>
                    <div style={{
                      width:"26px",height:"26px",borderRadius:"50%",background:"rgba(99,179,237,0.15)",
                      border:"1.5px solid rgba(99,179,237,0.35)",display:"flex",alignItems:"center",
                      justifyContent:"center",fontSize:"12px",fontWeight:"700",color:"#63b3ed",flexShrink:0
                    }}>{i+1}</div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:"13px",fontWeight:"600",color:"#f5f0e8",lineHeight:"1.4",marginBottom:"6px"}}>{fund.name}</div>
                      <div style={{display:"flex",alignItems:"center",gap:"8px",flexWrap:"wrap",marginBottom:"8px"}}>
                        <span style={{
                          fontSize:"11px",fontWeight:"600",padding:"2px 8px",borderRadius:"20px",
                          background:`${catColor}20`,color:catColor,border:`1px solid ${catColor}40`
                        }}>{fund.category}</span>
                        <span style={{fontSize:"11px",color:"#6a6258"}}>Regular · Growth</span>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                        <div style={{flex:1,height:"5px",background:"rgba(255,255,255,0.08)",borderRadius:"3px",overflow:"hidden"}}>
                          <div style={{height:"100%",width:`${pct}%`,background:barColor,borderRadius:"3px",transition:"width .6s ease"}}/>
                        </div>
                        <div style={{fontSize:"11px",fontWeight:"700",color:barColor,minWidth:"38px",textAlign:"right"}}>{pct}% match</div>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div style={{marginTop:"14px",padding:"10px 12px",background:"rgba(255,255,255,0.03)",borderRadius:"8px",fontSize:"11px",color:"#6a6258",lineHeight:"1.6"}}>
            ⚠️ These are suggestions based on your investor profile. Please verify fund details, expense ratio, and past performance on AMFI or Value Research before investing. All funds shown are Regular Plans — consider consulting a SEBI-registered advisor.
          </div>
        </div>
      )}
...
const QUESTIONS = [
  ...
  {
    id: "q2", num: "02", title: "Your Age (Beneficiary's Age)",
    sub: "Select your current age group.",
    type: "radio",
    options: [
  ...
  {
    id: "q3", num: "03", title: "Investment Horizon",
    sub: "How many years are you willing to stay invested? Be conservative.",
    type: "radio",
    options: [
      { value: "Under 1 year", label: "Under 1 year — Liquid / Ultra Short / Money Market" },
      { value: "1–3 years", label: "1 to 3 years — Short Duration / Debt" },
      { value: "3–7 years", label: "3 to 7 years — Hybrid / Balanced Advantage / Flexi-cap" },
      { value: "7–10 years", label: "7 to 10 years — Mid Cap / Large & Mid Cap" },
      { value: "10+ years", label: "More than 10 years — Small Cap / Sectoral / Thematic" },
    ],
  },
  {
    id: "q4", num: "04", title: "Investment Goal",
...
export default function App() {
  ...
  const isCombo23 = step === 1;  // Q2+Q3
  const isCombo45 = step === 3;  // Q4+Q5
  const isCombo67 = step === 5;  // Q6+Q7
  const isCombo89 = step === 7;  // Q8+Q9
  const isComboStep = isCombo23 || isCombo45 || isCombo67 || isCombo89;
  // 9 questions across 5 pages
  const effectiveStep = step <= 1 ? step : step <= 3 ? step-1 : step <= 5 ? step-2 : step <= 7 ? step-3 : step-4;
  const effectiveTotal = 5;
  const progress = (effectiveStep / effectiveTotal) * 100;

  const getValue = (id) => {
    if (id === "q1" && answers.q1 === "__HELP__") return "__HELP__";
    if (id === "q4" && answers.q4 === "Other") return otherText.trim() ? `Other: ${otherText.trim()}` : "";
    return answers[id] || "";
  };
  ...
  const canAdvance = () => {
    if (isCombo23) return getValue("q2") !== "" && getValue("q3") !== "";
    if (isCombo45) return getValue("q4") !== "" && getValue("q5") !== "";
    if (isCombo67) return getValue("q6") !== "" && getValue("q7") !== "";
    if (isCombo89) return getValue("q8") !== "" && getValue("q9") !== "";
  ...
  const goNext = () => {
    ...
    setError("");
    const nextStep = isComboStep ? step + 2 : step + 1;
    if (nextStep > total - 1) {
      const finalAnswers = { ...answers };
      if (answers.q4 === "Other") finalAnswers.q4 = otherText.trim() ? `Other: ${otherText.trim()}` : "Other";
      const r = scoreAnswers(finalAnswers);
      setResult({ answers: finalAnswers, ...r });
      if (finalAnswers.q1 === "__HELP__") {
        const sugg = suggestFunds(r.profile, finalAnswers);
        setSuggestions(sugg);
      }
    } else {
      setAnimDir("forward");
  ...
  const reset = () => {
    setStep(0); setAnswers({}); setOtherText(""); setError(""); setResult(null); setSuggestions(null); setAnimDir("forward");
  };
      ...
      <div style={{ maxWidth:"640px", margin:"0 auto", padding:"32px 20px 100px" }}>
        <div key={step} style={{ animation: `${animDir === "forward" ? "slideIn" : "slideInBack"} .35s ease` }}>
          ...
          {isCombo23 ? (
            <div>
              {/* Q2 Age */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>02</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Your Age (Beneficiary's Age)</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>Select your current age group.</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[1].options.map(opt => (<RadioOption key={opt.value} name="q2" value={opt.value} label={opt.label} selected={answers.q2===opt.value} onChange={v=>handleSelect("q2",v)} />))}
              </div>
              <div style={{ height:"1px", background:"rgba(255,255,255,0.06)", margin:"32px 0" }} />
              {/* Q3 Investment Horizon */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>03</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Investment Horizon</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>How many years are you willing to stay invested? Be conservative.</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[2].options.map(opt => (<RadioOption key={opt.value} name="q3" value={opt.value} label={opt.label} selected={answers.q3===opt.value} onChange={v=>handleSelect("q3",v)} />))}
              </div>
            </div>
            ...
            <div>
              {/* Q4 Investment Goal */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>04</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Investment Goal</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>What is this investment for?</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[3].options.map(opt => (<RadioOption key={opt.value} name="q4" value={opt.value} label={opt.label} selected={answers.q4===opt.value} onChange={v=>handleSelect("q4",v)} />))}
                {answers.q4 === "Other" && (
                  <div style={{ marginTop:"4px", animation:"fadeIn .2s ease" }}>
                    <input type="text" maxLength={100} value={otherText} onChange={e=>{setOtherText(e.target.value);setError("");}} placeholder="Describe your investment goal (max 100 characters)..." style={{ width:"100%", border:"1.5px solid rgba(212,160,23,0.4)", borderRadius:"10px", padding:"12px 14px", fontFamily:"inherit", fontSize:"14px", color:"#f5f0e8", background:"rgba(212,160,23,0.06)", outline:"none", boxSizing:"border-box" }} autoFocus />
              ...
              <div style={{ height:"1px", background:"rgba(255,255,255,0.06)", margin:"32px 0" }} />
              {/* Q5 Investment Mode */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>05</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Investment Mode</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>How do you plan to invest?</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[4].options.map(opt => (<RadioOption key={opt.value} name="q5" value={opt.value} label={opt.label} selected={answers.q5===opt.value} onChange={v=>handleSelect("q5",v)} />))}
              </div>
            </div>

          ) : isCombo67 ? (
            <div>
              {/* Q6 Existing Portfolio */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>06</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Existing Portfolio</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>How would you describe your current investment portfolio?</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[5].options.map(opt => (<RadioOption key={opt.value} name="q6" value={opt.value} label={opt.label} selected={answers.q6===opt.value} onChange={v=>handleSelect("q6",v)} />))}
              </div>
              <div style={{ height:"1px", background:"rgba(255,255,255,0.06)", margin:"32px 0" }} />
              {/* Q7 Risk & Volatility */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>07</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Risk & Volatility Appetite</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>How do you approach investment risk and short-term market swings?</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[6].options.map(opt => (<RadioOption key={opt.value} name="q7" value={opt.value} label={opt.label} selected={answers.q7===opt.value} onChange={v=>handleSelect("q7",v)} />))}
              </div>
            </div>
            ...
            <div>
              {/* Q8 Tax Sensitivity */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>08</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Tax Sensitivity</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>Which income tax bracket are you in?</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[7].options.map(opt => (<RadioOption key={opt.value} name="q8" value={opt.value} label={opt.label} selected={answers.q8===opt.value} onChange={v=>handleSelect("q8",v)} />))}
              </div>
              <div style={{ height:"1px", background:"rgba(255,255,255,0.06)", margin:"32px 0" }} />
              {/* Q9 Entry Context */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}><div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>09</div><div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/></div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(20px,4vw,28px)", fontWeight:"700", color:"#f5f0e8", lineHeight:"1.25", marginBottom:"8px" }}>Entry Context</div>
              <div style={{ fontSize:"14px", color:"#8a8278", marginBottom:"20px", lineHeight:"1.6" }}>What best describes your current situation with this fund?</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[8].options.map(opt => (<RadioOption key={opt.value} name="q9" value={opt.value} label={opt.label} selected={answers.q9===opt.value} onChange={v=>handleSelect("q9",v)} />))}
              </div>
            </div>
          ...
          {q.type === "fundsearch" && (<FundSearch value={answers.q1 || ""} onChange={v => handleSelect("q1", v)} />)}
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
              {q.options.map(opt => (
                <RadioOption key={opt.value} name={q.id} value={opt.value} label={opt.label}
                  selected={answers[q.id] === opt.value} onChange={v => handleSelect(q.id, v)} />
              ))}
              {q.hasOther && answers.q4 === "Other" && (
                <div style={{ marginTop:"4px", animation:"fadeIn .2s ease" }}>
                  <input type="text" maxLength={100} value={otherText} onChange={e=>{setOtherText(e.target.value);setError("");}} placeholder="Describe your investment goal (max 100 characters)..." style={{ width:"100%", border:"1.5px solid rgba(212,160,23,0.4)", borderRadius:"10px", padding:"12px 14px", fontFamily:"inherit", fontSize:"14px", color:"#f5f0e8", background:"rgba(212,160,23,0.06)", outline:"none", boxSizing:"border-box" }} autoFocus />
                  <div style={{ textAlign:"right", fontSize:"11px", color:"#8a8278", marginTop:"4px" }}>{otherText.length}/100</div>
                </div>
