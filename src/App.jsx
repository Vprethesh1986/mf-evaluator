...
function scoreAnswers(answers) {
  ...
  // Q7 Risk
  const riskMap = {
    "High — I am comfortable with significant short-term losses for higher long-term returns": 10,
    "Medium — I can accept moderate losses for moderate long-term gains": 6,
    "Low — I prioritize capital protection over high returns": 3,
  };
  scores.risk = riskMap[answers.q7] || 5;

  // Q8 Volatility
  const volMap = {
    "High — I am comfortable with sharp drawdowns if long-term compounding is strong": 10,
    "Medium — I can handle occasional sharp dips as long as the long-term trend is up": 6,
    "Low — I prefer a smooth, stable return curve even if that means lower returns": 3,
  };
  scores.volatility = volMap[answers.q8] || 5;
  ...
  scores.tax = taxMap[answers.q9] || 5;

  const weights = { age: 0.15, horizon: 0.20, mode: 0.10, portfolio: 0.10, risk: 0.20, volatility: 0.15, tax: 0.10 };
  ...
  } else if (answers.q9?.includes("20%")) {
    ...
    <div style={{maxWidth:"760px",margin:"0 auto",padding:"0 20px 80px",animation:"fadeIn .5s ease"}}>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"16px",padding:"28px",marginBottom:"28px"}}>
        ...
        {[
          ...
          ["Risk", answers.q7?.split("—")[0].trim()],
          ["Volatility", answers.q8?.split("—")[0].trim()],
          ["Tax Bracket", answers.q9?.split("—")[0].trim()],
          ["Context", answers.q10],
        ].map(([k, v]) => (
...
const QUESTIONS = [
  ...
  {
    id: "q7", num: "07", title: "Risk Tolerance",
    sub: "How do you feel about investment risk?",
    type: "radio",
    options: [
      { value: "Low — I prioritize capital protection over high returns", label: "Low — capital protection over high returns" },
      { value: "Medium — I can accept moderate losses for moderate long-term gains", label: "Medium — moderate losses for long-term gains" },
      { value: "High — I am comfortable with significant short-term losses for higher long-term returns", label: "High — significant short-term losses for higher returns" },
    ],
  },
  {
    id: "q8", num: "08", title: "Volatility Preference",
    sub: "How do you feel about short-term market swings?",
    type: "radio",
    options: [
      { value: "Low — I prefer a smooth, stable return curve even if that means lower returns", label: "Low — smooth curve even if lower returns" },
      { value: "Medium — I can handle occasional sharp dips as long as the long-term trend is up", label: "Medium — occasional dips if long-term trend is up" },
      { value: "High — I am comfortable with sharp drawdowns if long-term compounding is strong", label: "High — sharp drawdowns if compounding is strong" },
    ],
  },
  {
    id: "q9", num: "09", title: "Tax Sensitivity",
  ...
  {
    id: "q10", num: "10", title: "Entry Context",
      ...
      <div style={{ background:"rgba(0,0,0,0.6)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.06)", padding:"16px 40px" }}>
        <div style={{ maxWidth:"640px", margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontSize:"10px", letterSpacing:"1.5px", color:"#6a6258", marginTop:"3px" }}>Designed by{" "}<span style={{ color:"#d4a017", fontWeight:"600" }}>Vikram Prethesh (ViP)</span></div>
          </div>
