...
export default function App() {
  ...
  const effectiveTotal = total - 1; // Q3+Q4 share a page
  const effectiveStep = step > 2 ? step - 1 : step;
  ...
  const isComboStep = step === 2; // Q3+Q4 on same page
  const canAdvance = () => {
    if (isComboStep) return getValue("q3") !== "" && getValue("q4") !== "";
  ...
  const goNext = () => {
    ...
    setError("");
    // combo step: q3+q4 shown together, skip step 3 (q4 already answered)
    ...
    if (nextStep > total - 1) {
      ...
      setAnimDir("forward");
      setStep(isComboStep ? step + 2 : s => s + 1);
    }
  };
  ...
  const goBack = () => {
    if (step > 0) {
      setAnimDir("back");
      // if coming back from step 4 (q5), jump to combo step 2 (q3+q4)
      setStep(step === 4 ? 2 : s => s - 1);
      setError("");
    }
  };
    ...
    <div style={{ minHeight:"100vh", background:"#0d0d18", fontFamily:"'DM Sans', sans-serif", color:"#f5f0e8" }}>
      ...
      <div style={{ maxWidth:"640px", margin:"0 auto", padding:"32px 20px 100px" }}>
        <div key={step} style={{ animation: `${animDir === "forward" ? "slideIn" : "slideInBack"} .35s ease` }}>
          ...
          {/* ── COMBO STEP: Q3 + Q4 on same page ── */}
          {isComboStep ? (
            <div>
              {/* Q3 — Investment Horizon */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}>
                <div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>03</div>
              ...
              {/* Q4 — Investment Goal */}
              <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px" }}>
                <div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"rgba(212,160,23,0.12)", border:"1.5px solid rgba(212,160,23,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:"700", color:"#d4a017" }}>04</div>
                <div style={{ height:"1px", flex:1, background:"rgba(212,160,23,0.15)" }}/>
              </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {QUESTIONS[3].options.map(opt => (
                  <RadioOption key={opt.value} name="q4" value={opt.value} label={opt.label}
                    selected={answers.q4 === opt.value}
                    onChange={v => handleSelect("q4", v)} />
                ))}
                {answers.q4 === "Other" && (
                  <div style={{ marginTop:"4px", animation:"fadeIn .2s ease" }}>
                    <input type="text" maxLength={100} value={otherText}
                      onChange={e => { setOtherText(e.target.value); setError(""); }}
                      placeholder="Describe your investment goal (max 100 characters)..."
                      style={{ width:"100%", border:"1.5px solid rgba(212,160,23,0.4)", borderRadius:"10px", padding:"12px 14px", fontFamily:"inherit", fontSize:"14px", color:"#f5f0e8", background:"rgba(212,160,23,0.06)", outline:"none", boxSizing:"border-box" }}
                      autoFocus />
